...
export default function SnapPortfolio() {
    ...
    // Filter top 3 for the slider
    const slides = caseStudiesData.slice(0, 3);
        ...
        <section className="relative bg-[#020617]">
            <div ref={container} className="h-screen w-full relative overflow-hidden flex items-center">
                ...
                <div className="absolute top-8 left-8 z-10 hidden md:block">
                    <span className="text-xs font-mono text-secondary uppercase tracking-widest">Featured Works</span>
                </div>
                ...
                <div className="w-full h-full relative">
                    {slides.map((slide, i) => (
                        ...
                        >
                            {/* Big Number */}
                            <div className="absolute top-[10%] left-[5%] md:left-[15%] z-20 mix-blend-overlay pointer-events-none">
                            ...
                            <div className="absolute bottom-20 md:top-1/2 md:bottom-auto left-6 md:left-24 md:-translate-y-1/2 z-30 max-w-xl">
                                ...
                                <div className={`transform transition-all duration-700 delay-200 ${i === activeIndex ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
                                    <Link href={`/case-studies/${slide.slug}`} className="inline-flex items-center gap-4 text-white hover:text-secondary transition-colors group">
                                        <span className="text-xl font-bold uppercase tracking-wider">View Case Study</span>
                ...
                <div className="absolute bottom-8 right-8 z-40 flex gap-4">
                    {slides.map((slide, i) => (
                        <button
                            key={i}
                            onClick={() => {
                                // Scroll to specific section logic would be complex here due to ScrollTrigger scrubbing
                                // For now acts as indicator
                            }}
